...
The final ChatGPT response must be valid JSON only.
Do not include markdown, code fences, explanations, notes, or any text outside the JSON object.
...
- slug: URL-friendly lowercase slug using hyphens only.
- excerpt: 1-2 sentence blog summary for Wix.
- body: Full blog article body in plain text with clear headings and paragraphs.
...
- categories: 1-3 practical blog category names.
- featuredImagePrompt: A clear prompt for generating a professional featured image for the blog.
- socialPostLinkedIn: A professional LinkedIn post promoting the blog.
- socialPostFacebook: A simple Facebook post promoting the blog.
- emailSubject: A short Mailchimp-style email subject line.
- emailPreview: A short preview text for the email.
- emailBody: A short email campaign draft promoting the blog.
